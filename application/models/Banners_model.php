...
class Banners_model extends CI_Model
{
    ...
    {
        if (is_array($dados)) {
            $this->db->insert('app_banner', $dados);
        }
    }

    //Utilizar a ID do Banner para buscar
    public function getBannerID($id = NULL)
    {
        if ($id) {
            $this->db->where('id', $id);
            $this->db->limit(1);
            return $this->db->get('app_banner')->row();
        }
    }
    ...
    {
        if ($id) {
            $this->db->delete('app_banner', ['id' => $id]);
        }
    }
    ...
    public function atualizarBanner($dados = NULL, $condicao = NULL)
    {
        if (is_array($dados) && is_array($condicao)) {
            $this->db->update('app_banner', $dados, $condicao);
        }
    }
}
